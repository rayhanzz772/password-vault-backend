...
class Controller {
  static async createVaultPassword(req, res) {
    ...
    try {
      const { name, username, password, note, master_password, category_id } = req.body;
  ...
  static async updateVaultPassword(req, res) {
    ...
    try {
      const { id } = req.params;
      const { name, username, password, note, master_password, category } = req.body;
      const userId = req.user.userId;

      const item = await VaultPassword.findOne({ where: { id, user_id: userId } });
      ...
      if (password) {
        const encryptionResult = await encrypt(password, master_password);
